...
class GeminiClient(LLMClient):
    ...
    def get_completion(self, messages, max_retries=3, initial_retry_delay=1, **kwargs):
        """获取聊天完成结果，包含重试逻辑"""
        try:
            logger.info(f"⏳ 使用 Gemini 模型: {self.model}")
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if response is None:
                        logger.warning(
                            f"❌ 尝试 {attempt + 1}/{max_retries}: API 返回空值")
            ...
            logger.error(f"❌ get_completion 发生错误: {str(e)}")
            return None
...
class OpenAICompatibleClient(LLMClient):
    ...
    def __init__(self, api_key=None, base_url=None, model=None):
        ...
        if not self.base_url:
            logger.error("❌ 未找到 OPENAI_COMPATIBLE_BASE_URL 环境变量")
            raise ValueError(
                "OPENAI_COMPATIBLE_BASE_URL not found in environment variables")

        if not self.model:
            logger.error("❌ 未找到 OPENAI_COMPATIBLE_MODEL 环境变量")
            raise ValueError(
        ...
        logger.info("✅ OpenAI Compatible 客户端初始化成功")
    ...
    def call_api_with_retry(self, messages, stream=False, max_tokens=None):
        """带重试机制的 API 调用函数"""
        try:
            logger.info("⏳ 正在调用 OpenAI Compatible API...")
    ...
    def get_completion(self, messages, max_retries=3, initial_retry_delay=1, **kwargs):
        """获取聊天完成结果，包含重试逻辑"""
        try:
            logger.info(f"⏳ 使用 OpenAI Compatible 模型: {self.model}")
            logger.debug(f"消息内容: {messages}")
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.info("✅ 成功获取 OpenAI Compatible 响应")
        ...
        if client_type == "auto":
            ...
            if (kwargs.get("api_key") and kwargs.get("base_url") and kwargs.get("model")) or \
               (os.getenv("OPENAI_COMPATIBLE_API_KEY") and os.getenv("OPENAI_COMPATIBLE_BASE_URL") and os.getenv("OPENAI_COMPATIBLE_MODEL")):
                ...
                logger.info("⏳ 自动选择 OpenAI Compatible API")
            else:
                client_type = "gemini"
                logger.info("⏳ 自动选择 Gemini API")
