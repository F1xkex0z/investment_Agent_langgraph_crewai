...
def macro_news_agent(state: AgentState) -> Dict[str, Any]:
    ...
    logger.info(f"{agent_name}: 开始执行宏观新闻分析")
    ...
    logger.debug(f"{agent_name}: 初始化完成，目标指数: {symbol}")
    ...
    if os.path.exists(output_file_path):
        logger.debug(f"{agent_name}: 检查缓存文件: {output_file_path}")
        try:
            ...
            if today_str in all_summaries and all_summaries[today_str].get("summary_content"):
                ...
                logger.info(f"{agent_name}: 成功从缓存加载{today_str}的宏观新闻总结，新闻数量: {retrieved_news_count}")
        except json.JSONDecodeError:
            show_agent_reasoning(
                f"JSONDecodeError for {output_file_path} when trying to load cache. Will fetch fresh data.", agent_name)
            logger.error(f"{agent_name}: 缓存文件{output_file_path}格式错误，将获取新数据: {str(e)}")
            ...
            logger.error(f"{agent_name}: 加载缓存文件{output_file_path}错误，将获取新数据: {str(e)}")
            all_summaries = {}  # Reset on other errors

    if not from_cache:
        show_workflow_status(f"{agent_name}: 缓存中未找到今日总结或缓存无效，开始获取实时新闻。")
        logger.info(f"{agent_name}: 缓存中未找到{today_str}的总结或缓存无效，开始获取实时新闻")
        try:
            show_workflow_status(
                f"{agent_name}: Fetching news for symbol {symbol}")
            logger.debug(f"{agent_name}: 开始获取{symbol}的新闻数据")
            ...
            if news_df is None or news_df.empty:
                ...
                logger.warning(f"{agent_name}: 未获取到{symbol}的新闻数据")
                ...
                logger.info(f"{agent_name}: 成功获取到{symbol}的{retrieved_news_count}条新闻数据")
                logger.debug(f"{agent_name}: 开始处理新闻数据，准备LLM分析")
                ...
                logger.debug(f"{agent_name}: 新闻数据处理完成，共{len(news_list_for_llm)}条新闻准备用于LLM分析")
                ...
                logger.info(f"{agent_name}: 调用LLM进行宏观分析")
                logger.debug(f"{agent_name}: LLM提示词已准备完成，开始发送请求")
                ...
                logger.info(f"{agent_name}: LLM宏观分析结果获取成功，摘要预览: {summary[:100]}...")
            ...
            logger.error(f"{agent_name}: 获取新闻或执行分析时发生错误: {str(e)}")
    ...
    if not from_cache:  # Also save if summary was updated, even if initially from cache but e.g. re-analyzed
        ...
        logger.info(f"{agent_name}: 准备保存总结到{output_file_path}")
        ...
        if not os.path.exists(output_file_path) or 'all_summaries' not in locals():
            all_summaries = {}
            logger.debug(f"{agent_name}: 初始化all_summaries字典")
            # if file exists but all_summaries wasn't set (e.g. decode error)
            if os.path.exists(output_file_path):
                try:
                    with open(output_file_path, 'r', encoding='utf-8') as f:
                        all_summaries = json.load(f)
                    logger.debug(f"{agent_name}: 成功加载现有缓存文件")
                except json.JSONDecodeError:
                    all_summaries = {}  # If still error, start fresh
                    logger.warning(f"{agent_name}: 缓存文件格式错误，将创建新的缓存文件")

        os.makedirs(os.path.dirname(output_file_path),
                    exist_ok=True)  # Ensure directory exists
        logger.debug(f"{agent_name}: 确保目录存在: {os.path.dirname(output_file_path)}")
        ...
        logger.debug(f"{agent_name}: 更新缓存内容，日期: {today_str}")
    ...
    logger.debug(f"{agent_name}: 创建返回消息，消息内容长度: {len(new_message_content)}字符")
    ...
    logger.debug(f"{agent_name}: 准备元数据信息，新闻数量: {retrieved_news_count}, 缓存标志: {from_cache}")
